  ...
  <div class="applications-page">
      ...
      <div v-else v-for="application in filteredApplications" :key="application.id" 
        ...
        <div class="candidate-info">
          <div class="candidate-avatar">
            <img :src="getCandidateImage(application.candidate_id)" :alt="application.candidate_name">
          </div>
          <div>
        ...
        <div class="application-details">
          <p><i class="fas fa-calendar-alt"></i> Applied: {{ formatDate(application.applied_date) }}</p>
          <p><i class="fas fa-id-card"></i> ID: {{ application.id }}</p>
        </div>
    ...
    <div v-else-if="activeTab === 'accepted'" class="applications-container">
      ...
      <div v-else v-for="application in filteredAccepted" :key="application.id" 
        ...
        <div class="application-details">
          <p><i class="fas fa-calendar-alt"></i> Applied: {{ formatDate(application.applied_date) }}</p>
          <p><i class="fas fa-check-circle"></i> Accepted: {{ formatDate(application.accepted_date) }}</p>
        </div>
    ...
    <!-- Candidate Modal -->
    <div v-if="showCandidateModal" class="modal-overlay" @click.self="closeCandidateModal">
      <div class="modal-content candidate-modal">
        <button class="close-modal" @click="closeCandidateModal">&times;</button>
          ...
          <div class="profile-header-content">
            ...
            <div class="profile-info">
              ...
              <div class="profile-stats">
                ...
                <div class="stat-item">
                  <span class="stat-value">{{ selectedCandidate.location || 'Not specified' }}</span>
        ...
        <div class="candidate-sections">
          ...
          <div class="section">
            ...
            <div class="social-media-links">
              <a v-if="selectedCandidate.social_links && selectedCandidate.social_links.linkedin" :href="selectedCandidate.social_links.linkedin" class="social-link linkedin">
                <i class="fab fa-linkedin-in"></i>
              </a>
              <a v-if="selectedCandidate.social_links && selectedCandidate.social_links.twitter" :href="selectedCandidate.social_links.twitter" class="social-link twitter">
                <i class="fab fa-twitter"></i>
              </a>
              <a v-if="selectedCandidate.social_links && selectedCandidate.social_links.github" :href="selectedCandidate.social_links.github" class="social-link github">
                <i class="fab fa-github"></i>
              </a>
              <a v-if="selectedCandidate.social_links && selectedCandidate.social_links.portfolio" :href="selectedCandidate.social_links.portfolio" class="social-link portfolio">
...
export default {
  ...
  methods: {
    async fetchJob() {
      try {
        const response = await this.$axios.get(`/api/jobs/${this.id}`);
    ...
    async fetchApplications() {
      try {
        const response = await this.$axios.get(`http://localhost:8000/api/jobs/${this.id}/applications`);
    ...
    sortApps(apps) {
      switch(this.sortBy) {
        case 'newest':
          return apps.sort((a, b) => new Date(b.applied_date) - new Date(a.applied_date));
        case 'oldest':
          return apps.sort((a, b) => new Date(a.applied_date) - new Date(b.applied_date));
        case 'experience':
    ...
    getCandidateImage(candidateId) {
      const gender = candidateId % 2 === 0 ? 'women' : 'men';
      const id = candidateId % 100 || 44;
      return `https://randomuser.me/api/portraits/${gender}/${id}.jpg`;
    },
    
    viewCandidate(application) {
      this.selectedCandidate = application.candidate_data;
      this.selectedCandidate.id = application.candidate_id;
      this.selectedCandidate.name = application.candidate_name;
      this.selectedCandidate.position = application.position;
      this.selectedCandidate.experience = application.experience;
      this.selectedCandidate.education = application.education;
    ...
    async toggleShortlist(application) {
      try {
        ...
        await this.$axios.patch(`http://localhost:8000/api/applications/${application.id}`, {
          status: newStatus
        });
        ...
        this.$notify({
          title: 'Success',
          text: `Application ${newStatus === 'shortlisted' ? 'shortlisted' : 'removed from shortlist'}`,
          type: 'success'
        });
    ...
    async acceptCandidate(application) {
      try {
        ...
        application.accepted_date = new Date().toISOString();
        this.shortlistedApplications = this.allApplications.filter(app => app.status === 'shortlisted');
        this.acceptedApplications = this.allApplications.filter(app => app.status === 'accepted');
        
        this.$notify({
          title: 'Candidate Accepted',
          text: `${application.candidate_name} has been accepted for this position`,
          type: 'success'
        });
        ...
        this.$notify({
          title: 'Error',
          text: error.response?.data?.message || 'Failed to accept candidate',
          type: 'error'
        });
      }
    },
    ...
    async rejectCandidate(application) {
      try {
        ...
        this.$notify({
          ...
          text: `${application.candidate_name} has been rejected`,
          type: 'warn'
        });
        ...
        this.$notify({
          title: 'Error',
          text: error.response?.data?.message || 'Failed to reject candidate',
          type: 'error'
        });
      }
    },
